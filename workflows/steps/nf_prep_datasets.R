...
#
###########################################################################
# add input args to the script
library(argparse)
parser <- ArgumentParser()
parser$add_argument("--normalization", help="normalization method")
parser$add_argument("--geneset", help="geneset")
...
# Output directory:
experiment <- file.path(args$output, "prep_datasets")
outdir <- file.path('output', paste0(experiment))
dir.create(outdir, showWarnings = F)

# Would be nice to have one script that is going to test various normalization and transformation methods
