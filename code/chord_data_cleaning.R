...
dir.create(file.path(outdir, "lipid_therapy"), showWarnings = FALSE)
# Let"s look at the lipid lowering therapy on press scores
metadata$ID <- gsub("\\.[1-2]", "", rownames(metadata))
tmpDat <- metadata %>%
  group_by(ID) %>%
    filter(all(c("1", "2") %in% Study_Timepoint) & n_distinct(Intervention_group) == 1)
tmpDat <- tmpDat %>%
  group_by(ID) %>%
  mutate(diabetes = ifelse(Study_Timepoint == "2", lag(diabetes), diabetes))

with(tmpDat, table(ID, diabetes))
overall_t1_v_t2 <- tmpDat %>%
    filter(Intervention_group != "Repatha") %>%
    ggplot(aes(x = Study_Timepoint, y = press_score, fill = Study_Timepoint)) +
    facet_grid(~Intervention_group) +
    geom_boxplot(outlier.shape = NA) +
    geom_line(aes(group = ID), color = "black", alpha = 0.5) +
    stat_compare_means(method = "t.test", comparisons = list(c("1", "2")), paired = TRUE) +
    theme(legend.position = "none") +
    labs(x = "Timepoint", y = "PRESS Score")
ggsave(file.path(outdir, "chord_press_lipid_therapy.pdf"), plot = overall_t1_v_t2, width = 6, height = 6)

diabetic_t1_v_t2 <- tmpDat %>%
    filter(Intervention_group != "Repatha") %>%
    ggplot(aes(x = Study_Timepoint, y = press_score, rows = Study_Timepoint)) +
    facet_wrap(~interaction(diabetes, Intervention_group), ncol = 3) +
    geom_boxplot(outlier.shape = NA) +
    geom_point() +
    geom_line(aes(group = interaction(ID, Intervention_group)), color = "black", alpha = 0.5) +
    stat_compare_means(method = "t.test", paired = TRUE, vjust = 0.75) +
    labs(y = "PRESS Score", x = "Timepoint")
